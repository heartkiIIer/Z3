...
function addWakeById(req, res, id) {
    ...
    promise
        .then(function(internalId) {
            pool.query("UPDATE sleepentry SET terminate = current_timestamp WHERE user_id ="+internalId+" AND entry_id = SELECT MAX(entry_id) FROM sleepentry WHERE user_id ="+internalId+");" , (error, results) => {
                if (error) {
                    throw error
                }
                console.log(results.rows);
