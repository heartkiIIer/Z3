    ...
    selectComponent(){
        ...
        if(this.state.stage === -1 || this.state.routine == null){
            ...
            if(this.state.stage < this.state.stages){
                ...
                //Timer
                if(this.state.routine[this.state.stage].minutes !== 0) {
                    return <div><h1 className="alignTextRight">{this.state.stage.toString() + "/" + this.state.stages.toString()+ " tasks"}</h1> <BedtimeProgressBar key = {this.state.stage} id="items" title={this.state.routine[this.state.stage].task_name}
